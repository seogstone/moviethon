...
export default async function DeclinersRankingPage() {
  let payload: Awaited<ReturnType<typeof getLatestMoverRankings>> = { rows: [], asOfDate: null };
  try {
    payload = await getLatestMoverRankings({ type: "decliners", window: "7d", limit: 50 });
  } catch {
  ...
  return (
    <main className="mx-auto min-h-screen w-full max-w-[1440px] space-y-5 px-4 py-8 sm:px-6 lg:px-8">
      <div className="space-y-3 panel-shell rounded-2xl p-5 sm:p-6">
        ...
        <h1 className="text-3xl font-semibold text-[var(--foreground)]">top decliners (7d)</h1>
        <RankingsNav activeHref="/rankings/decliners" />
      </div>

      <RankingTable title="decliners" rows={payload.rows} emptyMessage="No decliner data yet." />
    </main>
  );
}
