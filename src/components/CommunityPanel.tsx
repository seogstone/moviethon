...
export function CommunityPanel({
  ...
  return (
    <section id="community" className="space-y-6 rounded-3xl border border-[#d9d7f2] bg-white p-5 shadow-[0_12px_26px_rgba(42,39,85,0.05)] sm:p-6">
      <div>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        ...
        <div className="space-y-3 rounded-2xl border border-[#e4e3f7] bg-[#f8f7ff] p-4">
          ...
          <button
            ...
            disabled={busy || !captchaToken || !displayName.trim() || !commentBody.trim()}
          ...
          {sortedComments.map((comment) => (
            <li key={comment.id} className="rounded-2xl border border-[#e4e3f7] bg-[#f8f7ff] p-3">
              <div className="flex items-start justify-between gap-2">
                <div>
                  <p className="text-sm font-medium text-[#1a1738]">{comment.displayName}</p>
